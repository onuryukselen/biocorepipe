...
//                $this->updateAmazonProStatus($id, "inactive", $ownerID);
    ...
    public function getParentSideBar($ownerID){
        ...
        $sql= "SELECT DISTINCT pg.group_name name, pg.id, pg.perms, pg.group_id
        FROM process_group pg
        INNER JOIN ( SELECT DISTINCT ug1.u_id FROM user_group ug1 INNER JOIN ( SELECT DISTINCT ug2.g_id FROM user_group ug2 where ug2.u_id ='$ownerID' ) b ON ug1.g_id = b.g_id) c ON c.u_id = pg.owner_id
    ...
    public function getPipelineSideBar($ownerID){
        if ($ownerID != ''){
          ...
          $sql= "SELECT DISTINCT pip.id, pip.name, pip.perms, pip.group_id, pip.pin
               ...
               INNER JOIN ( SELECT DISTINCT ug1.u_id FROM user_group ug1 INNER JOIN ( SELECT DISTINCT ug2.g_id FROM user_group ug2 where ug2.u_id ='$ownerID' ) b ON ug1.g_id = b.g_id) c ON c.u_id = pip.owner_id
               INNER JOIN (
                SELECT p.pipeline_gid, MAX(p.rev_id) rev_id
                FROM biocorepipe_save p
                INNER JOIN ( SELECT DISTINCT ug1.u_id FROM user_group ug1 INNER JOIN ( SELECT DISTINCT ug2.g_id FROM user_group ug2 where ug2.u_id ='$ownerID' ) b ON ug1.g_id = b.g_id) c ON c.u_id = p.owner_id
                $where
    ...
    public function getSubMenuFromSideBar($parent, $ownerID){
       ...
       $sql="SELECT DISTINCT p.id, p.name, p.perms, p.group_id
             ...
             INNER JOIN (
                SELECT pr.process_gid, MAX(pr.rev_id) rev_id
                FROM process pr
                INNER JOIN ( SELECT DISTINCT ug1.u_id FROM user_group ug1 INNER JOIN ( SELECT DISTINCT ug2.g_id FROM user_group ug2 where ug2.u_id ='$ownerID' ) b ON ug1.g_id = b.g_id) c ON c.u_id = pr.owner_id where $where_pr
                GROUP BY pr.process_gid
                ) b ON p.rev_id = b.rev_id AND p.process_gid=b.process_gid";
	...
	public function getSubMenuFromSideBarPipe($parent, $ownerID){
       ...
       $sql="SELECT DISTINCT p.id, p.name, p.perms, p.group_id
             ...
             INNER JOIN (
                SELECT pr.pipeline_gid, MAX(pr.rev_id) rev_id
                FROM biocorepipe_save pr
                INNER JOIN ( SELECT DISTINCT ug1.u_id FROM user_group ug1 INNER JOIN ( SELECT DISTINCT ug2.g_id FROM user_group ug2 where ug2.u_id ='$ownerID' ) b ON ug1.g_id = b.g_id) c ON c.u_id = pr.owner_id
                where $where_pr
                GROUP BY pr.pipeline_gid
                ) b ON p.rev_id = b.rev_id AND p.pipeline_gid=b.pipeline_gid";
    ...
    public function getParentSideBarProject($ownerID){
        ...
        FROM project pp
        INNER JOIN ( SELECT DISTINCT ug1.u_id FROM user_group ug1 INNER JOIN ( SELECT DISTINCT ug2.g_id FROM user_group ug2 where ug2.u_id ='$ownerID' ) b ON ug1.g_id = b.g_id) c ON c.u_id = pp.owner_id
